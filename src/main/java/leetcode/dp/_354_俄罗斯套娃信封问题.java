...
public class _354_俄罗斯套娃信封问题{
    ...
    // 这样的话算法的时间复杂度为 O(NlogN)，因为排序和计算 LIS 各需要 O(NlogN) 的时间，加到一起还是 O(NlogN)；空间复杂度为 O(N)，
    // 因为计算 LIS 的函数中需要一个 top 数组。
    ...
    class Solution {
        ...
        // 其次，两个 w 相同的新封不能相互包含，所以对于宽度 w 相同的信封，对高度 h 进行降序排序，保证LIS 中不存在多个w相同的信封
        // （题目中说了长宽相同也无法嵌套）
        // envelopes = [[w,h], [w,h]...]
        /**
         * <pre>
