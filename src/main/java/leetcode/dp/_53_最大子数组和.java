...
public class _53_最大子数组和{
    class Solution {
        ...
        // 这种情况不能利用数学归纳法根据 dp[i] 推出 dp[i+1],因为子数组一定是连续的，当前的dp数组定义，并不能保证 numss[0..i] 中最大子数组与 nums[i+1] 是相邻的，也就没办法推导出 dp[i+1].
        // 重新定义 dp[i] 表示以 nums[i] 为结尾的最大子数组和。这种定义下，不能直接返回 dp[n-1] 而需要遍历这个dp数组
        // 选择：要么与前面相邻子数组连接，形成一个更大的子数组；要么步与前面的子数组连接，自成一派，自己作为一个子数组。
