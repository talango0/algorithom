...
public class _53_最大子数组和{
    class Solution {
         ...
         * 这种情况不能利用数学归纳法根据 dp[i] 推出 dp[i+1],因为子数组一定是连续的，当前的dp数组定义，
         * 并不能保证 nums[0..i]中最大子数组与 nums[i+1] 是相邻的，也就没办法推导出 dp[i+1].
    ...
    class Solution2 {
        // 前缀和数组 preSum 就是 nums 元素的累加和 preSum[i+1] - preSum[j] 其实就是数组 nums[j..i] 之和.
        // （根据 preSum数组实现，索引0占位符，所以 i 有一位偏移）
