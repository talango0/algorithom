...
public class _787_K站中转内最便宜的航班{
    /*
    分析：
    出发点是 0，到达点是 2，允许的最大中转次数 K 为 1，所以最小的开销就是图中红色的两条边，从 0 出发，经过中转城市 1 到达目标城市 2，
    所以算法的返回值应该是 200。
    注意这个中转次数的上限 K 是比较棘手的，如果上述题目将 K 改为 0，也就是不允许中转，那么我们的算法只能返回 500 了，也就是直接从 0 飞到 2。
    很明显，这题就是个加权有向图中求最短路径的问题。
    说白了，就是给你一幅加权有向图，让你求 src 到 dst 权重最小的一条路径，同时要满足，这条路径最多不能超过 K + 1 条边
    （经过 K 个节点相当于经过 K + 1 条边）。
     */
    ...
    class Solution{
        // 求最短路径，肯定可以用到BFS算法解决
        // 因为 BFS 算法相当于从起始点开始，一步一步向外扩散，那当然是离起点越近的节点越先被遍历到，如果 BFS遍历的过程中遇到终点呢，那么走的肯定是最短路径。
        // 在多叉树（或者扩展到无权图）的遍历中，与其说没有权重，不如说是权重都是1， 起点和终点之间的路径权重就是它们之间边的条数。
        // 对于加权图，路径中的边的条数和路径的权重并不是正比关系了，需要用到优先级队列【自动排序】的特性，将路径权重较小的节点排在队列的最前面，
        // 以此为基础施展 BFS，也就变成了 Dijkstra 算法
