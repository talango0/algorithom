...
public class _130_被围绕的区域{
    ...
    class Solution2 {
        ...
        // 先用 for 循环遍历棋盘的四边，用DFS算法把那些与边界相连的 O 换成 # ，
        // 然后再遍历整个棋盘，把剩下的 O 换成 X ， 再把 # 换成 O。
        // 也可用 union-find算法解决，可以把那些 不需要被替换的 O 看出一个门派，
        // 它们有一个共同的【祖师爷】 dummy,这些 O 和 dummy互相联通，
        // 而那些需要被替换的 O 与 dummy不连通。
        public void solve(char[][] board) {
            // 二维坐标(x,y) 可以转换成 x*n+y 这个数（m是棋盘的行数，n是棋盘的列数）
            // ，这也是二维坐标映射到一维的常用技巧。
            // [0, m*n-1] 都是棋盘内坐标的一维映射，dummy 占据 m*n 的位置
