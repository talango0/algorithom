...
public class _410_分割数组的最大值{
    class Solution{
        ...
        // 所以我们不能用n == m而必须用n <= m来找到答案，因为如果你能把nums分割成 2 个子数组（[2],[1,1]），
        // 那么肯定也可以分割成 3 个子数组（[2],[1],[1]）。

        /**
         * 在每个子数组和不超过 max 的条件下，计算 nuns 至少可以分割成几个子数组，
         * <p>
         * 因此这种情况下，如果我们找到一个最小 max 值，满足 split(nums, max) 和 m 相等，
         * 那么这个最小的max 就是符合题意的【最小的最大子数组和】
         * <p>
         * 现在就简单了，我们只要对max进行穷举就行，那么最大子数组和max的取值范围是什么呢？
         * <p>
         * 显然，子数组至少包含一个元素，至多包含整个数组，所以「最大」子数组和的取值范围
         * 就是闭区间[max(nums), sum(nums)]，也就是最大元素值到整个数组和之间。
         * <p>
         * 时间复杂度分析：假设nums元素个数为N，元素和为S，则split函数的复杂度为O(N)，
         * 二分查找的复杂度为O(logS)，所以算法的总时间复杂度为O(N*logS)。
         */
