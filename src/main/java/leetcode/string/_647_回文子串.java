...
public class _647_回文子串{
    class Solution{
         ...
         * 1. 枚举所有的子串，然后判断这些子串是否时回文。O(n^2) 枚举子串 s[l_i, r_i] ，然后用O(r_i - l_i)
         * 时间判断是否回文串。方法1 O(n^3)
    ...
    class Solution2{
        ...
        public int countSubstrings(String s) {
            ...
            // f[i] 表示以 i 为回文中心的最长半径
            int[] f = new int[n];
            int iMax = 0,  // 当前最大回文串右端点对应的回文中心 i_m
                    rMax = 0,  // 当前的最大回文串右端点 r_m
                    ans = 0;
            for (int i = 1; i < n; i++) {
                // 初始化 f[i]
